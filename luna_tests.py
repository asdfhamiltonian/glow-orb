...
class AzAltTests(unittest.TestCase):
    ...
    def test_azalt_US(self):
        ...
        """
        azalt = self.location.calculate(2015, 8, 10, 9, 0, 0)
        azdif = abs(azalt[0] - 57.7800)
        altdif = abs(azalt[1] - (-5.7166))
        self.assertLess(azdif, 0.03)
        self.assertLess(altdif, 0.03)

        # 25 years later, year 2040:

        azalt = self.location.calculate(2040, 8, 10, 9, 0, 0)
        azdif = abs(azalt[0] - 341.3909)
        altdif = abs(azalt[1] - -33.0098)
        self.assertLess(azdif, 0.03)
        self.assertLess(altdif, 0.03)
    ...
    def test_azalt_JA(self):
        ...
        """
        azalt = self.location2.calculate(2015, 8, 10, 9, 0, 0)
        azdif = abs(azalt[0] - 322.4536)
        altdif = abs(azalt[1] - -27.5202)
        self.assertLess(azdif, 0.03)
        self.assertLess(altdif, 0.03)

        # 25 years later, year 2040:
        azalt = self.location2.calculate(2040, 8, 10, 9, 0, 0)
        azdif = abs(azalt[0] - 264.9162)
        altdif = abs(azalt[1] - 24.3637)
        self.assertLess(azdif, 0.03)
        self.assertLess(altdif, 0.03)
