...
def dec_to_time(dec):
    hr = math.floor(dec*24)
    mn = math.floor((dec-hr/24)*60*24)
    sec = round((dec-hr/24-mn/(60*24))*3600*24)
    print("sunrise/set at time " + str(hr) + "h" + str(mn) + "m" + str(sec) + "s")
...
def Clock1(a):
    ...
    time = city.wadokei()[0] #recalculates the wadokei time
